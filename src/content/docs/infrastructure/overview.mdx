...
This document provides a high-level overview of the infrastructure designed to deploy and manage containerized applications (specifically CTF challenges) dynamically. The core components include a private Docker registry for storing custom images, a K3s Kubernetes cluster for orchestration, a load balancer for traffic distribution and API access, and a custom FastAPI service to automate deployment workflows.
